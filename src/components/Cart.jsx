...
export default function cart({ cartItems, handleAddQuantity, handleRemoveQuantity, handleRemoveItem }) {
	return (
		<div class="font-sans max-w-5xl max-md:max-w-xl mx-auto bg-white py-4">
			...
			<div class="grid md:grid-cols-3 gap-8 mt-16">
				<div class="md:col-span-2 space-y-4">
					{cartItems.map((item) => (
						<div key={item.id} class="grid grid-cols-3 items-start gap-4">
							...
							<div class="ml-auto">
								<h4 class="text-lg max-sm:text-base font-bold text-gray-800">S${(item.price * item.quantity).toFixed(2)}</h4>
								...
								<div class="mt-6 flex items-center px-3 py-1.5 border border-gray-300 text-gray-800 text-xs outline-none bg-transparent rounded-md">
									<button type="button" onClick={() => handleRemoveQuantity(item.id)}>
									...
									<button type="button" onClick={() => handleAddQuantity(item.id)}>
				...
				<div class="bg-gray-100 rounded-md p-4 h-max">
					...
					<div class="mt-6 space-y-3">
						<button type="button" class="text-sm px-4 py-2.5 w-full font-semibold tracking-wide bg-gray-800 hover:bg-gray-900 text-white rounded-md">
							Checkout
						</button>
						<button
							type="button"
